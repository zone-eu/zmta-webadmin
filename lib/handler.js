...
function fetchCounters(zones, callback) {
    ...
    query.exec((err, replies) => {
        ...
        let i = 0;
        let totals = {};
        [].concat(replies || []).forEach(counter => {
...
function fetchZoneList(callback) {
    request(config.apiServer + '/zones', {
        json: true
        ...
        request(config.apiServer + '/counter/zone/', {
            json: true
        }, (error, response, data) => {
            if (error || response.statusCode !== 200) {
                return callback(error || new Error('Invalid response code ' + response.statusCode));
...
function fetchQueued(zone, type, callback) {
    request(config.apiServer + '/queued/' + encodeURIComponent(type) + '/' + encodeURIComponent(zone), {
        json: true
...
function fetchCounter(zone, callback) {
    request(config.apiServer + '/counter/zone/' + encodeURIComponent(zone), {
        json: true
    }, (error, response, data) => {
        if (error || response.statusCode !== 200) {
            return callback(error || new Error('Invalid response code ' + response.statusCode));
...
function fetchMessageData(id, callback) {
    request(config.apiServer + '/message/' + encodeURIComponent(id), {
        json: true
...
function fetchBlacklist(callback) {
    request(config.apiServer + '/blacklist', {
        json: true
    }, (error, response, data) => {
        if (error || response.statusCode !== 200) {
            return callback(error || new Error('Invalid response code ' + response.statusCode));
...
function fetchSuppressionlist(callback) {
    request(config.apiServer + '/suppressionlist', {
        json: true
    }, (error, response, data) => {
        if (error || response.statusCode !== 200) {
            return callback(error || new Error('Invalid response code ' + response.statusCode));
...
function addToSuppressionlist(payload, callback) {
    request.post(config.apiServer + '/suppressionlist', {
        json: payload
    }, (error, response, message) => {
        if (error || response.statusCode !== 200) {
...
function deleteFromSuppressionlist(payload, callback) {
    request.delete(config.apiServer + '/suppressionlist?id=' + encodeURIComponent(payload.id), {
        json: true
    }, (error, response, message) => {
        if (error || response.statusCode !== 200) {
            if (message && message.error) {
...
function postMessage(payload, callback) {
    request.post(config.apiServer + '/send-raw', {
        body: payload
    }, (error, response, message) => {
        if (error || response.statusCode !== 200) {
            try {
...
function deleteMessage(id, seq, callback) {
    request.delete(config.apiServer + '/message/' + id + '/' + (seq ? seq : ''), {
        json: true
    }, (error, response, info) => {
        if (error || response.statusCode !== 200) {
            if (info && info.error) {
...
function sendMessages(id, seq, callback) {
    request.put(config.apiServer + '/message/' + id + '/' + (seq ? seq : ''), {
        json: true
    }, (error, response, info) => {
        if (error || response.statusCode !== 200) {
            if (info && info.error) {
...
function fetchLogData(id, callback) {
    if (!config.logServer) {
        return callback(null, false);
    }
    request(config.logServer + '/get/' + encodeURIComponent(id), {
        json: true
    }, (error, response, message) => {
        if (error || response.statusCode !== 200) {
            if (response && response.statusCode === 404) {
...
function fetchMessageIdData(query, callback) {
    if (!config.logServer) {
        return callback(null, false);
    }
    request(config.logServer + '/find?messageId=' + encodeURIComponent(query), {
        json: true
    }, (error, response, message) => {
        if (error || response.statusCode !== 200) {
            if (response && response.statusCode === 404) {
